...
import CustomCheckbox from "../taskSheet/CustomCheckbox";
    ...
    <>
      <div className="border">
        <p className="text-center bg-smHeaderBg border-b border-subHeaderText flex items-center text-[8px] font-semibold">
          <span className="py-[9.5px] tracking-widest w-1/2 text-center">THIS MONTH'S GOALS</span>
            ...
            {Object.values(totalTargets).map((t, index) => (
              <div key={index} className="flex items-center gap-2">
                <CustomCheckbox
                  key={index}
                  checked={t.isDone}
                  onChange={(checked) => toggleCheckbox(index, checked)}
                  color="headerBg"
                  size={10}
                />
