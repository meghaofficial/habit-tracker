const HabitProgress = ({ progress, total, count }: { progress: { id: string, count: number, progress: string | number }[], total: number, count: number }) => {
  ...
  return (
    <div className="">
      ...
      <p className="text-center bg-darkPrimary light:bg-lightPrimary flex items-center text-white text-[8px]" style={{ fontWeight: "bold" }}>
      ...
      {progress?.map((p, index) => (
        <div className="text-[12px] flex items-center border-b border-darkBox/50 light:border-lightBorder  py-[5.6px]" key={index}>
          <div className="w-[70%] flex items-center gap-3 px-1 ps-3" title={p?.progress?.toString()}>
            <span className="w-[25%] text-[8px]">{p?.progress === '100.00' ? '100' : p?.progress}%</span>
            <div className="w-full">
              <div className="h-5 bg-darkSuccess light:bg-lightSuccess shadow-[0_0_5px_rgba(74,222,128,0.5)] rounded-sm" style={{ width: `${p?.progress}%` }}></div>
            </div>
          </div>
